...
def init_gaze_screen(scale=0.8,font_size=48,calibration_radius=300):
    ...
    return gestures, video_capture, screen, w, h, bold_font
...
def gaze_main_loop(gestures, video_capture, screen, screen_width, screen_height, bold_font, n_points=25, context="my_context"):
    ...
    eventos_mouse_real, eventos_mouse_com_tremor, eventos_mouse_suavizado, eventos_gaze, eventos_final = [], [], [], [], []

    # --- PARÂMETROS DE AJUSTE --------------------------
    p_chance_tremor = 1.5; 
    p_amplitude_tremor = 25
    p_suavizacao_mouse = 4          # média das últimas n posições para estabelecer centro estável
    p_mouse_speed_threshold = 15.0; # sensibilidade ao tremor para ativar 'freio'
    p_gaze_speed_threshold = 10.0   # diferenciar um olhar fixo de olhar em movimento
    p_lerp_pesado = 0.1;            # força do freio pesado, aplicada durante tremor. Move 10% em direção ao elvo (âncora)
    p_lerp_leve = 0.8               # força do freio leve, aplicada quando sem tremor. Move 60% em direção ao alvo
    # ---------------------------------------------------

    mouse_smoother = PositionSmoother(window_size=p_suavizacao_mouse)
    ...
    while running:
        ...
        if calibrate and calibration is not None:
            ...
            if capturing_input:
                ...
                pygame.mouse.set_visible(True)
                final_cursor_pos = mouse_pos_real

            smoothed_mouse_pos = mouse_smoother.smooth(mouse_pos_com_tremor[0], mouse_pos_com_tremor[1])
            ...
            if gaze_event is not None and gaze_event.point is not None:
                ...
                cursor_lerp_factor = p_lerp_leve
                if mouse_speed > p_mouse_speed_threshold and gaze_speed < p_gaze_speed_threshold:
                    cursor_lerp_factor = p_lerp_pesado 
                
                target_pos = smoothed_mouse_pos
            ...
            if capturing_input:
                ...
                eventos_mouse_suavizado.append({'timestamp': timestamp, 'x': smoothed_mouse_pos[0], 'y': smoothed_mouse_pos[1]})
    ...
    df_mouse_suavizado = pd.DataFrame(eventos_mouse_suavizado)
    df_gaze = pd.DataFrame(eventos_gaze)
    df_final = pd.DataFrame(eventos_final)
    
    # Quantidade de registros capturados
    print(f'Registros: Real={len(df_mouse_real)}, Tremor={len(df_mouse_com_tremor)}, Suavizado={len(df_mouse_suavizado)}, Gaze={len(df_gaze)}, Final={len(df_final)}')

    # Salvar dados
    df_mouse_real.to_pickle("df_mouse_real.pkl")
    df_mouse_com_tremor.to_pickle("df_mouse_com_tremor.pkl")
    df_mouse_suavizado.to_pickle("df_mouse_suavizado.pkl")
